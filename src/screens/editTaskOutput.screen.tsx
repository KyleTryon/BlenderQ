...
const variableDefinitions: { [key in keyof PathVariables]: { description: string } } = {
...
const EditTaskOutputScreen: FC<Params> = ({ taskIndex }) => {
    ...
    const handleSubmit = () => {
        ...
        const buildVars = (task: typeof tasks[0], idx: number): PathVariables => ({
    ...
    return (
        <DefaultLayout commands={[enterCommand]}>
            ...
            <Box flexDirection='column' borderColor={'yellow'} borderStyle={'round'}>
                ...
                {Object.entries(variableDefinitions).map(([key, { description }]) => (
                    <Text key={key}>
                        <Text color="green">${`{${key}}`}</Text> - {description}
                    </Text>
                ))}
                <Text>
                    Example: <Text color="cyan">/output/${`{NAME}`}_${`{TASK_INDEX}`}/</Text>
                </Text>
            </Box>
        </DefaultLayout>
    );
