...
type QueueTaskStatus = 'INITIALIZING' | 'QUEUED' | 'RUNNING' | 'COMPLETED' | 'FAILED'
...
export const QueueScreen: React.FC<QueueScreenProps> = ({ blendFiles }) => {
    ...
    useEffect(() => {
        const newTasks = blendFiles.map((blendFile): QueueTask => ({
            enabled: true,
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setTableData(tasks.map(({ blendFile, enabled, ...rest }) => ({
                ...rest,
                enabled: enabled ? '◉' : '○',
            })))
        }, 500)
